...
export async function getCopilotToken(): Promise<string> {
  ...
  for (let attempt = 0; attempt < rows.length; attempt++) {
    ...
    try {
      ...
      await supabase.from("ai_tokens").update({ status: "dead", last_tested_at: new Date().toISOString() }).eq("id", row.id);
      CACHE.delete(row.id);
      continue;
    }
  }
  throw new Error("All Copilot tokens are dead or exhausted. Add a new token in Admin → AI Tokens.");
}
...
export function getCopilotBaseURL(inferenceToken: string): string {
  try {
    ...
    if (proxyEp) return proxyEp.replace("proxy.", "api.");
  } catch {}
